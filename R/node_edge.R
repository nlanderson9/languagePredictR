...
#' @param binaryPenalty If TRUE, edge weights for each word pair will be penalized according to the edge weight of that word pair in the opposing text dataset. See Details.
...
#'}
#'
node_edge = function(input, maxDist=4, removeStopwords=FALSE, binaryPenalty=FALSE, showProgress = TRUE) {
  ...
  if (class(input) == "langModel") {
    if (input@type == "binary") {
      ...
      if (binaryPenalty) {
        intermediate_zero = level0_result %>% left_join(level1_result, by=c("first", "second"))
        intermediate_zero$weight.y = ifelse(is.na(intermediate_zero$weight.y), 1, intermediate_zero$weight.y)

        intermediate_zero$weight.x = intermediate_zero$weight.x * (1 - intermediate_zero$weight.y)

        intermediate_one = level1_result %>% left_join(level0_result, by=c("first", "second"))
        intermediate_one$weight.y = ifelse(is.na(intermediate_one$weight.y), 1, intermediate_one$weight.y)

        intermediate_one$weight.x = intermediate_one$weight.x * (1 - intermediate_one$weight.y)

        intermediate_zero = subset(intermediate_zero, select = c(first, second, inverse_mean_dist.x, cooc_count.x, first_count.x, second_count.x, weight.x, outcome.x))
        colnames(intermediate_zero) = c("first", "second", "inverse_mean_dist", "cooc_count", "first_count", "second_count", "weight", "outcome")
        intermediate_one = subset(intermediate_one, select = c(first, second, inverse_mean_dist.x, cooc_count.x, first_count.x, second_count.x, weight.x, outcome.x))
        colnames(intermediate_one) = c("first", "second", "inverse_mean_dist", "cooc_count", "first_count", "second_count", "weight", "outcome")

        repeated_word_table2 = bind_rows(intermediate_zero, intermediate_one, overall_result)
      }
      else {
